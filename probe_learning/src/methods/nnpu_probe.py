"""nnPU probe: non-negative PU learning with labeled negatives (PNU setting)."""
...
def nnpu_loss(
    ...
    """nnPU loss for PNU setting.

    L = L_pos(P) + L_neg(N) + clamp(L_neg(U) - pi * L_pos_as_neg(P), min=0)
    """
    ...
    return L_pos + L_neg_clean + neg_risk_nn


def train_one_attribute_nnpu(
    attr: str,
    ...
    """Train MLP with nnPU loss (PNU setting)."""
    _seed_everything(seed)
    X_train, X_val, y_train, y_val = split_labeled_validation(X_labeled, y_labeled, seed, validation_data)
    if pi is None:
        # Estimate the class prior from the actual fitting partition only.  The
        # held-out validation labels must not influence the training loss.
    ...
    for epoch in range(1, epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            if pos_mask.any() and neg_mask.any():
                loss = nnpu_loss(logits_pn[pos_mask], logits_pn[neg_mask], logits_u, pi)
            elif pos_mask.any():
                L_pos = F.binary_cross_entropy_with_logits(
